...
// Copyright (c) 2009-2012 The CAOGENCOIN developers
...
bool RecvLineFixed(SOCKET hSocket, string& strLine)
{
    ...
    loop
    {
        ...
        // +CAOGENCOIN added for irc exits, without this, recv(hSocket, &c, 1, 0); will hang when closing the program after using irc on windows.
		bool stop = false;
		u_long RecvCount;
        while(!stop)
        {
...
void ThreadIRCSeed()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #M3GAC01NTEST3\r");
            Send(hSocket, "WHO #M3GAC01NTEST3\r");
        } else {
            // randomly join #M3GAC01N00-#M3GAC01N99
            int channel_number = GetRandInt(100);
			channel_number = 1; // For now, just use one channel
            Send(hSocket, strprintf("JOIN #CAOGENCOIN20%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #CAOGENCOIN20%02d\r", channel_number).c_str());
        }
