...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non CGC unit input in lnReqAmount, we generate the URI with CGC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::CGC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
